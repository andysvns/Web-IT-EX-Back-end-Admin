  ...
  export default {
    ...
    data() {
      return {
        ...
        toolNameRules: [
          ...
          v => (v && v.length <= 50) || "Tool name must be less than 50 characters"
        ],
    ...
    methods: {
      ...
      async fetchStackTypes() {
        try {
          const response = await axios.get("http://localhost:3000/api/stacktype/getall/");
      ...
      async createItem() {
        ...
        try {
          const response = await axios.post("http://localhost:3000/api/stacktool/create", formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          });
          if (response.data.success) {
            this.showSnackbar('success', 'Stack tool created successfully');
            setTimeout(() => {
              this.$router.go(-1);
            }, 1500);
          } else {
            this.showSnackbar('error', response.data.message || 'Failed to create stack tool');
          }
        } catch (err) {
          console.error("Failed to create stack tool:", err);
          this.showSnackbar('error', 'Failed to create stack tool. Please try again.');
        }
      },

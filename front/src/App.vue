<template>
  <div>
    <v-app>
      <v-navigation-drawer v-model="outerDrawer"  app class="side-bar-nav" v-if="!isLoginPage && !isRegisterPage">
    ...
    <v-list nav>
      <template v-for="item in menuItems">
        ...
        >
          ...
          >
            <v-list-item-content>
              <v-list-item-title class="title-text">{{ subitem.title }}</v-list-item-title>
            </v-list-item-content>
          </v-list-item>
        </v-list-group>
      ...
      <div class="top-navbar" v-if="!isLoginPage && !isRegisterPage">
        <v-app-bar-nav-icon @click="outerDrawer = !outerDrawer" class="drawer-icon" style="color: #000000; display: show"  />
        <h1>
          {{ appTitle }}
        </h1>
  ...
  export default {
    ...
    data () {
      ...
      menuItems: [
          ...
          subitems: [
            { title: 'List Users', path: '/users/list' },
